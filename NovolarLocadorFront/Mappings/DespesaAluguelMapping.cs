        ...
        public DespesaAluguelMapping()
        {
			try
			{
				CreateMap<DespesaImovel, DespesaReadDTO>()
					...
					.ForMember<decimal>(e => e.ValorDespesa, options => options.MapFrom(input => string.IsNullOrEmpty(input.vl_valor_imod) ? 0.0M : decimal.Parse(input.vl_valor_imod, CultureInfo.InvariantCulture)))
